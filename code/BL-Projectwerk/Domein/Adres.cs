...
namespace BL_Projectwerk.Domein {
    public class Adres 
    {
        ...
        public void ZetNummer(string nummer) 
        {
            ...
            if (!Controle.IsGoedeAdresNummerSyntax(nummer)) { throw new AdresException("Adres - ZetNummer - Geen geldig nummer ingevuld"); }
            Nummer = nummer.Trim();
        }
        ...
        {
            return $"Adres: {Id} - {Straat} - {Nummer} - {Postcode} - {Plaats} - {Land}";
        }
