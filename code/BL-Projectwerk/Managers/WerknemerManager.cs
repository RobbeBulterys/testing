...
namespace BL_Projectwerk.Managers {
    public class WerknemerManager 
    {
        ...
        public void VoegWerknemerToe(Werknemer werknemer) 
        {
            try 
            {
                if (werknemer == null) throw new WerknemerManagerException("WerknemerManager - VoegWerknemerToe - Werknemer is null");
        ...
        public bool BestaatWerknemer(int werknemerId) 
        {
            try 
            {
                if (werknemerId == 0) throw new WerknemerManagerException("WerknemerManager - BestaatWerknemer - Ongeldige werknemerid");
        ...
        public bool BestaatWerknemer(string naam, string voornaam) 
        {
            try 
            {
                if (string.IsNullOrEmpty(naam) || string.IsNullOrEmpty(voornaam)) throw new WerknemerManagerException("WerknemerManager - BestaatWerknemer - Naam en/of voornaam is niet ingevuld");
        ...
        public void VerwijderWerknemer(int werknemerId) 
        {
            try 
            {
                if (werknemerId == 0) throw new WerknemerManagerException("WerknemerManager - VerwijderWerknemer - Ongeldige werknemerid"); 
                if (!_werknemerRepo.BestaatWerknemer(werknemerId)) throw new WerknemerManagerException("WerknemerManager - VerwijderWerknemer - Onbestaande werknemerid"); 
        ...
        public void UpdateWerknemer(int werknemerId, string? naam, string? voornaam) 
        {
            try 
            {
                if (string.IsNullOrWhiteSpace(naam) && string.IsNullOrWhiteSpace(voornaam)) throw new WerknemerManagerException("WerknemerManager - UpdateWerknemer - Geen updates ingevuld");
                if (!string.IsNullOrWhiteSpace(naam) || !string.IsNullOrWhiteSpace(voornaam)) {
                    if (_werknemerRepo.BestaatWerknemer(werknemerId)) {
                        ...
                        if (dbwerknemer.IsDezelfde(update)) throw new WerknemerManagerException("WerknemerManager - UpdateWerknemer - Werknemer is hetzelfde");
                    }
                }
        ...
        public IReadOnlyList<Werknemer> ZoekWerknemers(string? naam, string? voornaam) 
        {
            ...
            try 
            {
                if (!string.IsNullOrEmpty(naam) || !string.IsNullOrEmpty(voornaam)) werknemers.AddRange(_werknemerRepo.GeefWerknemers(naam, voornaam));
                else throw new WerknemerManagerException("WerknemerManager - ZoekWerknemers - Geen velden ingevuld");
                return werknemers;
            } 
            catch (Exception ex) 
            {
        ...
        public Werknemer GeefWerknemer(int werknemerId)
        {
            ...
            try
            {
                if (werknemerId == 0 || werknemerId < 0) { throw new WerknemerManagerException("WerknemerManager - GeefWerknemer - Ongeldig werknemerid"); }
