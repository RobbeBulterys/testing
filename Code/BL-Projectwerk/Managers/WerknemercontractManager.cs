...
namespace BL_Projectwerk.Managers {
    public class WerknemercontractManager {
        ...
        public bool BestaatContract(int werknemerId, int bedrijfsId) {
            try {
                if (werknemerId <= 0 || bedrijfsId <= 0) { throw new WerknemercontractManagerException("BestaatContract - ongeldige id's"); }
        ...
        public void VerwijderContract(Werknemercontract contract) {
            try {
                if (contract == null) { throw new WerknemercontractManagerException("VerwijderContract - Geen contract ingevuld"); }
                if (!_wcRepo.BestaatContract(contract.Werknemer.PersoonId, contract.Bedrijf.Id)) { throw new WerknemercontractManagerException("VerwijderContract - Contract bestaat niet"); }
        ...
        public void VoegContractToe(Werknemercontract contract) {
            try {
                if (contract == null) { throw new WerknemercontractManagerException("VoegContractToe - Geen contract gegeven"); }
                if (_wcRepo.BestaatContract(contract.Werknemer.PersoonId, contract.Bedrijf.Id)) { throw new WerknemercontractManagerException("VoegContractToe - Werknemercontract bestaat al"); }
